	...
	while index < aliasData.len() {
		...
		match(retVal){
			...
			None => {	
						...
						{ test_logger.fail(  ~"test_aliases.rs", copy external_argument, ~"parserutils", ~"parserutils.rs", ~"parserutils__charset_alias_canonicalise", copy aliasData[index], ~"Some value", ~"No value", ~"")
						}
					}
			}														
			index += 1;
