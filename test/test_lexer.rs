 ...
 fn main()
{
				...
				while(true)
                {
                	...
                	match(Errr)
                	{
                		LEXER_NEEDDATA => {
                            if tokOpt.is_some() {
                                tok= tokOpt.unwrap();
                                test1.pass( ~"lexer",~"css_lexer.rs"  , ~"file reading", ~"test_lexer" , fmt!("token read is---NEED DATA---- %?",tok )) ;
                	...
                	test1.pass( ~"lexer",~"css_lexer.rs"  , ~"file reading", ~"test_lexer" , fmt!("token read is %?",tok )) ;
 ...
 //                	match(Errr)
 //                	{
 //                		LEXER_NEEDDATA => break,
 //                			_=>{}
 //                	}
 //                	let tok= match(tokOpt)
 //                	{
 //                		Some(tok)=>tok,
 //                		None=> {/*io::println("none block")*/ ;break}
 //                	};
 //                	//if(!x.is_none()){break;}
	// 				io::println(fmt!("token=%?",tok));
	// 				match(tok  )
	// 				{
	// 					CSS_TOKEN_EOF  =>{/*io::println("none block")*/ ;break},
	// 					_=>{}
	// 				}	
 //                }
