...
fn main () {
	...
	let mut lwc = lwc_single_threaded();
	...
	while i<1000000 {		
		while j<50 {
			lwc.lwc_intern_string(strings[j]);
			j += 1;
		}
		i += 1;
	}
	...
	j=0 ;
	while j<50 {
		interned_strings_orig.push( lwc.lwc_intern_string(strings[j]) );
		j += 1;
	}
	
	j=0 ;
	while j<50 {
		interned_strings_copy.push( lwc.lwc_intern_string(strings[j]) );
		j += 1;
	}

	j=0 ;
	while j<50 {
		interned_strings_lcase.push( lwc.lwc_intern_string(strings[j]) );
		j += 1;
	}
	...
	while i<1000000 {		
		while j<50 {
			assert!(lwc.lwc_string_caseless_isequal( interned_strings_orig[j] , interned_strings_lcase[j] ) );
			j += 1;
		}
		i += 1;
	}
