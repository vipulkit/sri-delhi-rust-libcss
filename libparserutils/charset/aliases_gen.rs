...
impl alias_gen {
    ...
    fn write_alias(&mut self) {
        ...
        self.write_mibenum_is_unicode(fp);

        self.write_alias_impl(fp);

        self.write_trim_search_alias(fp);
        self.write_alias_map_find(fp);
        self.write_mibenum_map_find(fp);
        self.write_parserutils_charset_mibenum_from_name(fp);
        self.write_parserutils_charset_mibenum_to_name(fp);
        self.write_parserutils__charset_alias_canonicalise(fp);
        
        self.write_alias_impl_end(fp);

        self.write_alias_function(fp);
    ...
    fn write_header(&mut self, fp:@Writer) {
        fp.write_line("/*");
        fp.write_line(" * This file was generated by LibParserutils aliases_gen ");
        fp.write_line(" * Mozilla Samsung Servo Browser Project.");
        fp.write_line(" */");
        fp.write_line("");
        fp.write_line("extern mod std;");
        fp.write_line("use std::arc;");
        fp.write_line("");
        fp.write_line("pub struct parserutils_charset_aliases_canon {");
        fp.write_line("    mib_enum:u16,");
        fp.write_line("    name_len:u16,");
        fp.write_line("    name: ~str");
        fp.write_line("}");
        fp.write_line("");
    ...
    fn write_memcmp(&mut self, fp:@Writer) {
        fp.write_line("");
        fp.write_line("pub fn memcmp(str1 : &[u8] , str2 : &[u8] , len : uint ) -> int {");
        fp.write_line("    let mut i : uint = 0 ;");
        fp.write_line("    while ( i<len ) {");
        fp.write_line("        if str1[i] != str2[i] {");
        fp.write_line("            return ( (str1[i]-str2[i]) as int) ;");
        fp.write_line("        }");
        fp.write_line("        i = i+1 ; ");
        fp.write_line("    }");
        fp.write_line("    0");
    ...
    fn write_mibenum_is_unicode(&mut self, fp:@Writer) {
        fp.write_line("");
        fp.write_line("pub fn mibenum_is_unicode(mibenum:u16) -> bool {");
        fp.write_line("    (mibenum == 1000) || ");
        fp.write_line("    (mibenum == 1001) || ");
        fp.write_line("    (mibenum == 1015) || ");
        fp.write_line("    (mibenum == 1013) || ");
        fp.write_line("    (mibenum == 1014) || ");
        fp.write_line("    (mibenum == 1017) || ");
        fp.write_line("    (mibenum == 1018) || ");
        fp.write_line("    (mibenum == 1019) || ");
        fp.write_line("    (mibenum == 106)");
        fp.write_line("}");
        fp.write_line("");
    }

    fn write_dummy_main(&mut self, fp:@Writer) {
        fp.write_line("");
        fp.write_line("fn main() {}");
        fp.write_line("");
    }

    fn write_alias_impl(&mut self, fp:@Writer) {
        fp.write_line("");
        fp.write_line("impl alias {");
        fp.write_line("");
    }

    fn write_alias_impl_end(&mut self, fp:@Writer) {
        fp.write_line("");
        fp.write_line("} // impl alias");
        fp.write_line("");
    }
    ...
    fn write_trim_search_alias(&mut self, fp:@Writer) {
        ...
        fp.write_line(""    );
        fp.write_line("        for str::each_chari(alias) |index, c| {");
        ...
        fp.write_line("        }");
        fp.write_line("");
        fp.write_line("        for str::each_chari_reverse(alias) |index, c| {");
        fp.write_line("            if char::is_alphanumeric(c) {");
        ...
        fp.write_line("        }");
        fp.write_line("");
        fp.write_line("        let trimmed_alias = str::slice(alias, trim_left_index, trim_right_index).to_owned();");
        fp.write_line("        trimmed_alias");
        fp.write_line("    }");
        fp.write_line("");
    }

    fn write_alias_map_find(&mut self, fp:@Writer) {
        fp.write_line("");
    ...
    fn write_mibenum_map_find(&mut self, fp:@Writer) {
        fp.write_line("");
    ...
    fn write_parserutils__charset_alias_canonicalise(&mut self, fp:@Writer) {
        ...
        fp.write_line("                                    name: temp_name,");
        fp.write_line("                                    name_len: temp_name_len");
        fp.write_line("                                }");
        fp.write_line("                            )");
        fp.write_line("                        }");
        fp.write_line("                        else {");
    ...
    fn write_parserutils_charset_mibenum_to_name(&mut self, fp:@Writer) {
        fp.write_line("");
        fp.write_line("    pub fn parserutils_charset_mibenum_to_name(&self, mibenum: u16)-> Option<~str> {");
        fp.write_line("        match self.mibenum_map_find(mibenum) {");
        fp.write_line("            None => None,");
        fp.write_line("            Some (canonical_name_list_index) => {");
        fp.write_line("                if canonical_name_list_index < self.canonical_name_list.len() {");
