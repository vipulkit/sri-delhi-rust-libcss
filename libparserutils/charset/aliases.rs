...
impl alias {
    ...
    fn trim_search_alias(alias: ~str) -> ~str {
        //io::println("Entering: trim_search_alias");
        //io::println(alias);
        
        let mut trim_left_index = 0u;
        let mut trim_right_index = alias.len()-1;
        
        for str::each_chari(alias) |index, c| {
        ...
        for str::each_chari_reverse(alias) |index, c| {
            if char::is_alphanumeric(c) {
                trim_right_index = index+1;
                break;
            }
        }

        //io::println(fmt!("trim_left_index == %?", trim_left_index));
        //io::println(fmt!("trim_right_index == %?", trim_right_index));

        let trimmed_alias = str::slice(alias, trim_left_index, trim_right_index).to_owned();

        //io::println(fmt!("trimmed_alias == %?", trimmed_alias));
        trimmed_alias
    }

    pub fn parserutils__charset_alias_canonicalise(&self, alias: ~str) 
        -> Option<parserutils_charset_aliases_canon> {       
        match self.alias_map.find(&alias::trim_search_alias(alias.to_lower())) {         
            None => None,                   
            Some(temp_mib_enum) => {
