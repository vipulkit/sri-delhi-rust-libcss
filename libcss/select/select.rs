...
//        println(fmt!("Entering _selector_next")) ;
   ...
   //         println("2");
    ...
    pub fn match_selectors_in_sheet(&mut self, sheet : @mut css_stylesheet, 
        ...
        let mut id_slot = -1;
        let mut id_index = -1;
	let mut element_index:uint;
	let mut element_slot:uint;
        //let mut error : css_error ;
...
//        node_selectors_hash_entry = sel;
        ...
        if ( state.classes.len() != 0 ) {
            ...
            while z<z_len {
                ...
                if index_class != -1 {
                class_selectors_index_list.push(point {x:slot_class, y:index_class}) ;
                }
                z += 1;
            }
        }
       ...
       //     println("after call to _selector_next");
            ...
            if ( node_selectors_option.is_some() &&
                ...
                if next_element_index != -1 {
                    ...
                    node_selectors_option = Some((sheet.selectors.elements[element_slot][element_index]));
                }
                else {
                ...
                id_slot = id_slot_next;
                id_index = id_next_index;
            ...
            else if (slot_univ != -1 && index_univ != -1 && mut_ptr_eq(selector, univ_selectors_option.get())){
                let (univ_next_slot, univ_next_index) = 
                            sheet.selectors._iterate_universal(slot_univ, index_univ);

                slot_univ = univ_next_slot;
                index_univ = univ_next_index;

                if slot_univ != -1 && index_univ != -1 {
                   univ_selectors_option = Some(sheet.selectors.universal[slot_univ][index_univ]);    
                }
                else
                {
                ...
                while i < n_classes  {
                    ...
                    if ( x != -1 && y != -1 && mut_ptr_eq(selector, sheet.selectors.classes[x][y])) 
                    {
                        ...
                        class_selectors_index_list[i].x = slot_class;
                        class_selectors_index_list[i].y = index_class;
                        break;
                    }
