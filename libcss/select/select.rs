        ...
        match (detail.selector_type) {
            CSS_SELECTOR_ELEMENT => {
                if (detail.negate) {
                     ...
                     * otherwise. */
                    error = (state.handler.get_ref().node_has_name)(&mut self.lwc_ref, state.pw.unwrap(), node,
                            detail.qname, matched);
                }
            }
            CSS_SELECTOR_CLASS => {
                error = (state.handler.get_ref().node_has_class)(&mut self.lwc_ref, state.pw.unwrap(), node,
                        lwc_name , matched);
            }       
            CSS_SELECTOR_ID => {
                error = (state.handler.get_ref().node_has_id)(&mut self.lwc_ref, state.pw.unwrap(), node,
                        lwc_name , matched);
            }
